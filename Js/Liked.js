...
document.addEventListener('keydown', function (event) {
    ...
    } else if ((event.ctrlKey || event.metaKey) && event.key === 's') { // Shuffle
...
document.querySelectorAll('#songrow').forEach(row => {
    row.addEventListener('click', function (e) {
        // For Song
        let song = document.getElementById(this.querySelector('audio').id);
        ...
        if (song.paused) {
            ...
            this.querySelector('i').classList.remove('fa-pause-circle');
            this.querySelector('i').classList.add('fa-play-circle');
            mplaypause.src = '../Css/Icons/play.png';
            mplaypause.style.transition = "all .5s ease";
        }
        currentSong = song;
        document.querySelector('#likeImage').src = '../Css/Icons/Liked.png';
        document.querySelector('.playlist').src = '../Css/Icons/Add.png';
        document.querySelector('#liketext').innerText = 'Like';
    });
...
document.querySelectorAll('#songrow').forEach(row => {
    row.addEventListener('click', function () {
        ...
        rightCoverImg.style.transition = "all 0.5s ease";
...
shuffle.addEventListener('click', () => {
    if (currentSong) {
        ...
        setTimeout(() => {
            ...
            selectedRow.style.transition = 'box-shadow 0.3s ease';
        }, 1000);
    }
});
