...
setInterval(() => {
    ...
    if (Ctime) Ctime.innerText = displayTime(time);
    if (Dtime) Dtime.innerText = displayTime(duration);
...
document.querySelectorAll('#songrow').forEach(row => {
    row.addEventListener('click', function () {
        ...
        if (minicover) {
            ...
            minicover.style.transition = "all 0.5s ease";
        }

        if (hovercoverimage) {
            hovercoverimage.src = cover.src;
            hovercoverimage.style.animation = "songcover 0.5s ease";
        }

        hovercoverimage.style.animation = "songcoverclose 0.5s ease";
...
previous.addEventListener('click', () => {
    ...
    if (currentIndex === 0) {
        alert("No Previous Song. This is the First Song");
    } else {
...
function OnSocialmedia(social) {
    ...
    switch (social.toLowerCase()) {
        ...
        case "linkedin":
            ...
            break;
        case "whatsapp":
            openurl = isMobile ? `https://api.whatsapp.com/send?text=${quote} ${url}` : alert("WhatsApp sharing works only on mobile. Please use the WhatsApp app.");
            break;
        case "telegram":
            openurl = isMobile ? `https://t.me/share/url?url=${url}&text=${quote}` : alert("Telegram sharing works only on mobile. Please use the Telegram app.");
            break;
        case "github":
        ...
        case "skype":
            ...
            break;
        case "snapchat":
            openurl = isMobile ? `https://www.snapchat.com/scan?attachmentUrl=${url}` : alert("Snapchat sharing works only on mobile. Please use the Snapchat app.");
            break;
        case "pinterest":
        ...
        case "reddit":
            ...
            break;
        default:
            console.error(`Share button for ${social} is not implemented.`);
            return;
    }
