...
if (!empty($_SESSION['LoggedInUserID']) && !empty($_SESSION['Username'])) {
    $sql = "SELECT * FROM AsUsers WHERE User_Id = ?";
    if ($stmt = $conn->prepare($sql)) {
        $stmt->bind_param("i", $_SESSION['LoggedInUserID']);
        $stmt->execute();
...
<head>
    ...
    <title>Music4U : Top of Music4U </title>
...
<body>
            ...
            <div class="container">
                ...
                <div class="center">
                    <table>
                        ...
                        $array = array(8, 14, 16, 54, 55, 65, 67, 80, 81, 83, 86, 89, 90, 94, 124, 134, 135, 136, 140, 142, 143, 149, 152, 158, 161, 162, 166, 167, 168, 169, 170, 171, 172, 173, 174, 175,176);
                        $placeholders = implode(',', $array);
                        $sql = "SELECT * FROM songs WHERE Music_Id IN ($placeholders) ORDER BY RAND()";
                        $result = mysqli_query($conn, $sql);
                        while ($row = mysqli_fetch_assoc($result)) {
    ...
    <script>
        let hasWelcomeAlertBeenShown = false;
        document.addEventListener('DOMContentLoaded', function() {
            const checkInterval = 2000;
            const contentElementId = "dataset";
                ...
                return navigator.mediaDevices.enumerateDevices()
                    .then(devices => {
                        ...
                        if (audioOutputs.length === 0 || !audioOutputs[0].label) {
                            return requestPermissionsAndRetry();
                        }
            ...
            function requestPermissionsAndRetry() {
            ...
            function updateUI(hasHeadphones) {
                ...
                if (hasHeadphones) {
                    ...
                    document.body.style.overflow = "auto";
                    Swal.fire = '';
                    if (typeof localStorage !== 'undefined' && localStorage.getItem('hasWelcomeAlertBeenShown') !== 'true') {
                        hasWelcomeAlertBeenShown = true;
                        localStorage.setItem('hasWelcomeAlertBeenShown', 'true');
                        Swal.fire = '';
                    }
                } else {
                    ...
                    if (typeof Swal !== 'undefined') {
                        Swal.fire({
                            icon: 'error',
                            title: 'Headphones Detection Failed',
                            text: 'Please Use 🎧 Headphones to Listen Best on Music4U',
                            customClass: {
                                container: 'swal-container-small',
                                popup: 'swal-popup-black',
                                title: 'swal-title-size'
            ...
            function handleHeadphoneStatus() {
                detectHeadphones().then(hasHeadphones => {
                    if (headphonesConnected !== hasHeadphones) {
                        headphonesConnected = hasHeadphones;
                        updateUI(hasHeadphones);
                    }
                });
            }

            handleHeadphoneStatus();
            if (navigator.mediaDevices) {
                navigator.mediaDevices.addEventListener("devicechange", handleHeadphoneStatus);
            } else {
                setInterval(handleHeadphoneStatus, checkInterval);
            }
        });
    </script>
