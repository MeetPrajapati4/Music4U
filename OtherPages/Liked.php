...
if (isset($_SESSION['LoggedInUserID']) && !empty($_SESSION['LoggedInUserID'])) {
    ...
    $sql = "SELECT * FROM liked WHERE Liked_User_Id = ?";
        ...
        <div class="container">
            ...
            <div class="center">
                <table>
                    ...
                    $sql = "SELECT * FROM songs s JOIN Liked l ON s.Music_Id = l.Liked_Music_Id WHERE l.Liked_User_Id = ? ORDER BY l.Liked_Id DESC";
                    $stmt = $conn->prepare($sql);
                    $stmt->bind_param("i", $_SESSION['LoggedInUserID']); // Assuming session contains LoggedInUserID
                    $stmt->execute();
                    $result = $stmt->get_result();
                    while ($row = mysqli_fetch_assoc($result)) {
                ...
                <script>
                    document.addEventListener("DOMContentLoaded", () => {
                        ...
                        likeBtn.addEventListener('click', () => {
                            const isDisliked = likeBtn.children[0].name === "disliked";
                            if (!isDisliked) return;

                            const songId = document.querySelector('.focus #songId').value;
                            const userId = <?php echo json_encode($_SESSION['LoggedInUserID']); ?>;
                            const songName = document.querySelector('.focus #songname').textContent;

                            const formData = new URLSearchParams();
                            formData.append('songid', songId);
                            formData.append('userid', userId);

                            fetch('DislikeSong.php', {
                                    method: 'POST',
                                    headers: {
                                        'Content-Type': 'application/x-www-form-urlencoded',
                                    },
                                ...
                                .then(data => {
                                    const success = data.includes("success");
                                    Swal.fire({
                                        title: `You Disliked ${songName}`,
                                        icon: 'info',
                                        confirmButtonText: 'Ok',
                                        customClass: {
                                            container: 'swal-container-small',
                                    ...
                                    if (success) {
                                        likeBtn.children[0].name = "disliked";
                                        likeBtn.children[0].src = "../Css/Icons/Disliked.png";
                                        likeBtn.children[1].textContent = "Dislike";
                                    }
                                })
                                .catch(error => {
                                    Swal.fire({
